...
namespace KBEngine
{
	...
	public class DATATYPE_AVATAR_INFOS : DATATYPE_BASE
	{
		...
		public AVATAR_INFOS createFromStreamEx(MemoryStream stream)
		{
			...
			datas.modelID = stream.readUint32();
			datas.spaceKey = stream.readUint64();
		...
		public void addToStreamEx(Bundle stream, AVATAR_INFOS v)
		{
			...
			stream.writeUint32(v.modelID);
			stream.writeUint64(v.spaceKey);
	...
	public class DATATYPE_SPACE_REPORT : DATATYPE_BASE
	{
		public SPACE_REPORT createFromStreamEx(MemoryStream stream)
		{
			SPACE_REPORT datas = new SPACE_REPORT();
			datas.spaceKey = stream.readUint64();
			datas.spaceName = stream.readUnicode();
			datas.spaceUType = stream.readUint32();
			datas.avatarCount = stream.readUint8();
			datas.maxPlayer = stream.readUint8();
			return datas;
		}

		public void addToStreamEx(Bundle stream, SPACE_REPORT v)
		{
			stream.writeUint64(v.spaceKey);
			stream.writeUnicode(v.spaceName);
			stream.writeUint32(v.spaceUType);
			stream.writeUint8(v.avatarCount);
			stream.writeUint8(v.maxPlayer);
		}
	}



	public class DATATYPE_AVATAR_REPORT : DATATYPE_BASE
	{
		public AVATAR_REPORT createFromStreamEx(MemoryStream stream)
		{
			AVATAR_REPORT datas = new AVATAR_REPORT();
			datas.avatarName = stream.readUnicode();
			datas.avatarState = stream.readInt8();
			datas.avatarSubstate = stream.readUint8();
			datas.avatarPosition = stream.readVector3();
			return datas;
		}

		public void addToStreamEx(Bundle stream, AVATAR_REPORT v)
		{
			stream.writeUnicode(v.avatarName);
			stream.writeInt8(v.avatarState);
			stream.writeUint8(v.avatarSubstate);
			stream.writeVector3(v.avatarPosition);
		}
	}
	...
	{
		private DATATYPE_SPACE_REPORT itemType = new DATATYPE_SPACE_REPORT();

		public List<SPACE_REPORT> createFromStreamEx(MemoryStream stream)
		{
			UInt32 size = stream.readUint32();
			List<SPACE_REPORT> datas = new List<SPACE_REPORT>();
		...
		public void addToStreamEx(Bundle stream, List<SPACE_REPORT> v)
		{
			stream.writeUint32((UInt32)v.Count);
			for(int i=0; i<v.Count; ++i)
			{
	...
	{
		private DATATYPE_AVATAR_REPORT itemType = new DATATYPE_AVATAR_REPORT();

		public List<AVATAR_REPORT> createFromStreamEx(MemoryStream stream)
		{
			UInt32 size = stream.readUint32();
			List<AVATAR_REPORT> datas = new List<AVATAR_REPORT>();
		...
		public void addToStreamEx(Bundle stream, List<AVATAR_REPORT> v)
		{
			stream.writeUint32((UInt32)v.Count);
			for(int i=0; i<v.Count; ++i)
			{
