/*
	Generated by KBEngine!
	Please do not modify this file!
	
	tools = kbcmd
*/

namespace KBEngine
{
	using UnityEngine;
	using System;
	using System.Collections;
	using System.Collections.Generic;

	// defined in */scripts/entity_defs/Avatar.def
	public class EntityBaseEntityCall_AvatarBase : EntityCall
	{

		public EntityBaseEntityCall_AvatarBase(Int32 eid, string ename) : base(eid, ename)
		{
			type = ENTITYCALL_TYPE.ENTITYCALL_TYPE_BASE;
		}

	}

	public class EntityCellEntityCall_AvatarBase : EntityCall
	{

		public EntityCellEntityCall_AvatarBase(Int32 eid, string ename) : base(eid, ename)
		{
			type = ENTITYCALL_TYPE.ENTITYCALL_TYPE_CELL;
		}

		public void alive()
		{
			Bundle pBundle = newCall("alive", 0);
			if(pBundle == null)
				return;

			sendCall(null);
		}

		public void changeStates(SByte arg1, Byte arg2)
		{
			Bundle pBundle = newCall("changeStates", 0);
			if(pBundle == null)
				return;

			bundle.writeInt8(arg1);
			bundle.writeUint8(arg2);
			sendCall(null);
		}

		public void getNeighbours()
		{
			Bundle pBundle = newCall("getNeighbours", 0);
			if(pBundle == null)
				return;

			sendCall(null);
		}

		public void jump()
		{
			Bundle pBundle = newCall("jump", 0);
			if(pBundle == null)
				return;

			sendCall(null);
		}

		public void relive(Byte arg1)
		{
			Bundle pBundle = newCall("relive", 0);
			if(pBundle == null)
				return;

			bundle.writeUint8(arg1);
			sendCall(null);
		}

		public void reqToken()
		{
			Bundle pBundle = newCall("reqToken", 0);
			if(pBundle == null)
				return;

			sendCall(null);
		}

		public void sendEmoji(Byte arg1)
		{
			Bundle pBundle = newCall("sendEmoji", 0);
			if(pBundle == null)
				return;

			bundle.writeUint8(arg1);
			sendCall(null);
		}

		public void sendInfo(UInt32 arg1, UInt32 arg2)
		{
			Bundle pBundle = newCall("sendInfo", 0);
			if(pBundle == null)
				return;

			bundle.writeUint32(arg1);
			bundle.writeUint32(arg2);
			sendCall(null);
		}

		public void sendString(string arg1)
		{
			Bundle pBundle = newCall("sendString", 0);
			if(pBundle == null)
				return;

			bundle.writeUnicode(arg1);
			sendCall(null);
		}

	}
	}
