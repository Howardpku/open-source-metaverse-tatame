...
namespace KBEngine
{
	...
	public abstract class SpaceBase : Entity
	{
		...
		public virtual void onMaxPlayerChanged(Byte oldValue) {}
		...
		public virtual void onSpaceNameChanged(string oldValue) {}
		public UInt32 uid = 0;
			...
			while(stream.length() > 0)
			{
				...
				switch(prop.properUtype)
				{
					case 40001:
						...
						break;
					case 15:
						Byte oldval_maxPlayer = maxPlayer;
						maxPlayer = stream.readUint8();

						if(prop.isBase())
						{
							if(inited)
								onMaxPlayerChanged(oldval_maxPlayer);
						}
						else
						{
							if(inWorld)
					...
					case 40002:
						...
						break;
					case 14:
						string oldval_spaceName = spaceName;
						spaceName = stream.readUnicode();

						if(prop.isBase())
						{
							if(inited)
								onSpaceNameChanged(oldval_spaceName);
						}
						else
						{
							if(inWorld)
		...
		public override void callPropertysSetMethods()
		{
			...
			Byte oldval_maxPlayer = maxPlayer;
			Property prop_maxPlayer = pdatas[4];
			if(prop_maxPlayer.isBase())
			{
				if(inited && !inWorld)
					onMaxPlayerChanged(oldval_maxPlayer);
			}
			else
			{
				if(inWorld)
				{
					if(prop_maxPlayer.isOwnerOnly() && !isPlayer())
					{
					}
					else
					{
			...
			string oldval_spaceName = spaceName;
			Property prop_spaceName = pdatas[8];
			if(prop_spaceName.isBase())
			{
				if(inited && !inWorld)
					onSpaceNameChanged(oldval_spaceName);
			}
			else
			{
				if(inWorld)
				{
					if(prop_spaceName.isOwnerOnly() && !isPlayer())
					{
					}
					else
					{
